...
from ocap.grouping import agglomerative_groups, incremental_split_merge_groups


class GroupingTest(unittest.TestCase):
    def test_positive_pairs_merge_until_minimum_group_count(self):
        representations = {
            "a": np.array([1.0, 0.0]),
            "b": np.array([0.9, 0.1]),
            "c": np.array([0.0, 1.0]),
        ...
        result = agglomerative_groups(list(representations), representations, minimum_group_count=2)
        self.assertEqual(len(result.clusters), 2)
        self.assertEqual(result.stop_reason, "minimum_group_count")
    ...
    def test_negative_similarity_stops_before_target(self):
        ...
        result = agglomerative_groups(list(representations), representations, minimum_group_count=1)
        self.assertEqual(len(result.clusters), 2)
    ...
    def test_incremental_grouping_reuses_partition_and_merges_new_user(self):
        ...
        result = incremental_split_merge_groups(
            list(representations), representations, [{"a", "b"}, {"c", "d"}],
            minimum_group_count=2, max_split_merge_swaps=0,
        )
    ...
    def test_incremental_split_merge_requires_cohesion_improvement(self):
        ...
        result = incremental_split_merge_groups(
            list(representations), representations, [{"a", "b"}, {"c", "d"}],
            minimum_group_count=2, split_min_users=2, max_split_merge_swaps=1,
        )
