...
def first_k_metrics(
    events: Iterable[Mapping[str, object]], ks: Sequence[int], require_complete: bool = True,
) -> List[Dict[str, float]]:
    ...
    for k in sorted(set(int(v) for v in ks)):
        ...
        for user_events in by_user.values():
            if require_complete and len(user_events) < k:
                continue
            subset = user_events[:k]
            if not subset:
                continue
            included_users += 1
        ...
        rows.append({
            ...
            "n_events": float(len(pooled_y)),
            "user_macro_f1": float(np.mean(macro["f1"])) if macro["f1"] else float("nan"),
            "user_macro_recall": float(np.mean(macro["recall"])) if macro["recall"] else float("nan"),
            "user_macro_specificity": float(np.mean(macro["specificity"])) if macro["specificity"] else float("nan"),
            "user_macro_balanced_accuracy": float(np.mean(macro["balanced_accuracy"])) if macro["balanced_accuracy"] else float("nan"),
            "pooled_f1": pooled["f1"],
