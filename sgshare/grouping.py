...
def centroid(members: Iterable[str], signatures: Mapping[str, np.ndarray]) -> np.ndarray:
    rows = [
        np.asarray(signatures[user], dtype=np.float64).reshape(-1)
        for user in sorted(members) if user in signatures
    ]
    if not rows:
        raise ValueError("Cannot calculate a centroid without signatures")
...
def agglomerative_groups(
    ...
    if not ordered:
        return AgglomerationResult([], 0, "no_signatures", float("nan"))
    clusters: Dict[int, Set[str]] = {
        index: {user} for index, user in enumerate(ordered)
    }
    centroids: Dict[int, np.ndarray] = {
        index: np.asarray(signatures[user], dtype=np.float64).reshape(-1).copy()
...
def binary_split(members: Set[str], signatures: Mapping[str, np.ndarray]) -> tuple[Set[str], Set[str]] | None:
    users = sorted(user for user in members if user in signatures)
    if len(users) < 2:
        return None
    farthest = None
    for i in range(len(users)):
        for j in range(i + 1, len(users)):
            distance = 1.0 - cosine(signatures[users[i]], signatures[users[j]])
    ...
    right: Set[str] = set()
    for user in users:
        sim_a = cosine(signatures[user], signatures[seed_a])
        sim_b = cosine(signatures[user], signatures[seed_b])
