...
class TrainingConfig:
    ...
    route_grad_ema_decay: float = 0.95
    route_grad_normalize_before_ema: bool = False
    backbone_lr_scale: float = 0.3
    route_grad_clip_norm: float = 1.0
...
class GroupingConfig:
    ...
    warmup_signature_enabled: bool = True
    warmup_initial_grouping_enabled: bool = True
    warmup_initial_min_samples: int = 5
    warmup_initial_min_samples_fallback: int = 1
    warmup_initial_min_eligible_users_fallback: int = 2
    warmup_signature_count: int = 10
    warmup_min_eligible_users: int = 10
    warmup_fallback_signature_count: int = 1
    warmup_fallback_min_users: int = 2
    ...
    warmup_signature_keep_after_initial: bool = False
    prototype_init_enabled: bool = True
    prototype_init_min_samples: int = 3
    prototype_init_alpha: float = 0.35
    cold_start_safe_mixing: bool = True
    cold_start_mixing_until: int = 20
    cold_start_mixing_eta: float = 0.5
...
class RefinementConfig:
    ...
    mature_refine: bool = True
    mature_min_observations: int = 20
    mature_min_buffer: int = 5
...
def load_config(path: str | Path | None = None, dataset: str = "synthetic") -> ExperimentConfig:
    if path is None:
        if dataset == "synthetic":
            ...
            cfg.model.ff_mult = 2
            cfg.grouping.global_warmup_steps = 20
            cfg.grouping.individual_warmup_steps = 2
            cfg.grouping.min_observations = 4
            cfg.grouping.regroup_interval = 20
            cfg.grouping.warmup_signature_count = 2
            cfg.grouping.warmup_min_eligible_users = 4
