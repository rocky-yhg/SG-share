...
class TransformerBackbone(nn.Module):
    """Transformer backbone used by OCAP."""
    ...
    def __init__(self, input_dim: int, cfg: ModelConfig) -> None:
        ...
        self.blocks = nn.ModuleList([
            TransformerBlock(dim, cfg.num_heads, cfg.ff_mult, cfg.dropout)
            for _ in range(cfg.depth)
        ])
        self.norm = nn.LayerNorm(dim)
        self.gradient_projection = nn.Parameter(torch.zeros(dim, dim))
        self.gradient_projection_enabled = True
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, list[torch.Tensor]]:
        ...
        if self.gradient_projection_enabled:
            z = z + z @ self.gradient_projection.t()
        return z, attention


class OCAPModel(nn.Module):
    def __init__(self, input_dim: int, cfg: ModelConfig, device: str = "cpu") -> None:
        super().__init__()
        self.cfg = cfg
        self.backbone = TransformerBackbone(input_dim, cfg)
    ...
    def blend_adapter(self, destination: str, adapter_source: str, alpha: float) -> None:
        dst = self.clone_adapter_state(destination)
        src = self.clone_adapter_state(adapter_source)
        self.set_adapter_state(destination, {
